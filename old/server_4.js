...
//            console.log(JSON.stringify(games));
    ...
    socket.on('joinGame', function (username, roomcode) {
        if(username.length > 0) {            
            socket.username = username;        
            for (var z in games) {                    
                    if (games[z].room_id === roomcode && games[z].active === false) {
                        socket.join(roomcode);
                        socket.room = roomcode;
...
//                        io.to(roomcode).emit('servermsg', msg, roomcode);
                        io.to(roomcode).emit('updateGameLobby', roomcode, games);                        
                        console.log(JSON.stringify(games));                        
                    } else if (games[z].room_id === roomcode && games[z].active === true) {
                        socket.join(roomcode);
    //                    socket.room = roomcode;
            ...
            if (games[x].room_id === room) {
                for (var y in games[x].players) {
                    if (games[x].players[y].id === socket.username) {
                        ...
                        io.to(room).emit('responseLogged', response);
                    } else if (games[x].host.id === socket.username) {
                        games[x].host.answers.push(response);
                        io.to(room).emit('responseLogged');
                    }
                }
            }               
            if (games[x].players.length > 0) {
                for (var y in games[x].players) {
                    console.log(games[x].players[y].answers);
                }
            };
        }
    });
