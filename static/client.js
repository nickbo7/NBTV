...
socket.on('beginVote', function(games) {
    ...
    for (var x in games) {
        if (games[x].room_id === room) {
            if (host === true) {
                for (var y in games[x].players) {
                    answerList.push("<li><button class=\"voteButton\">" + games[x].players[y].answers[0] + "</button></li>");
                }
            } else if (player === true) {
                answerList.push("<li><button class=\"voteButton\">" + games[x].host.answers[0] + "</button></li>");
                for (var y in games[x].players) {
                    if (games[x].players[y].id === username) {
                    } else {                        
                        answerList.push("<li><button class=\"voteButton\">" + games[x].players[y].answers[0] + "</button></li>");
...
ul.onclick = function(event) {
    ...
    var buttons = document.getElementsByClassName("voteButton");
